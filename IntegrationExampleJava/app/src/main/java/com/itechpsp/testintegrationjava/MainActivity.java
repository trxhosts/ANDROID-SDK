package com.trxhosts.testintegrationjava;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TRXHOSTSPaymentInfo paymentInfo = getPaymentInfoOnlyRequiredParams(); // getPaymentInfoAllParams
...
//        theme.showShadow = false;
        ...
        // Present Checkout UI
        startActivityForResult(TRXHOSTSActivity.buildIntent(this,
                paymentInfo
//                ,theme
                ),
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == PAY_ACTIVITY_REQUEST) {
            ...
            switch (resultCode) {
                case TRXHOSTSActivity.RESULT_SUCCESS:
                case TRXHOSTSActivity.RESULT_CANCELLED:
                case TRXHOSTSActivity.RESULT_DECLINE:
                case TRXHOSTSActivity.RESULT_FAILED:
                    break;
            }

            if(data != null && data.hasExtra(TRXHOSTSActivity.DATA_INTENT_EXTRA_ERROR)) {
                String error = data.getStringExtra(TRXHOSTSActivity.DATA_INTENT_EXTRA_ERROR);
            }

            if(data != null && data.hasExtra(TRXHOSTSActivity.DATA_INTENT_EXTRA_TOKEN)) {
                String token = data.getStringExtra(TRXHOSTSActivity.DATA_INTENT_EXTRA_TOKEN);
            }
        }
    }

    // Payment Info
    TRXHOSTSPaymentInfo getPaymentInfoOnlyRequiredParams() {
        return new TRXHOSTSPaymentInfo(
    ...
    TRXHOSTSPaymentInfo getPaymentInfoAllParams() {
        return new TRXHOSTSPaymentInfo(
                PROJECT_ID, // project ID that is assigned to you
                "internal_payment_id_1", // payment ID to identify payment in your system
                1999, // 19.99
    ...
    // Additional
    void setDMSPayment(TRXHOSTSPaymentInfo paymentInfo) {
        paymentInfo.setAction(TRXHOSTSPaymentInfo.ActionType.Auth);
    }

    void setActionTokenize(TRXHOSTSPaymentInfo paymentInfo) {
        paymentInfo.setAction(TRXHOSTSPaymentInfo.ActionType.Tokenize);
    }

    void setActionVerify(TRXHOSTSPaymentInfo paymentInfo) {
        paymentInfo.setAction(TRXHOSTSPaymentInfo.ActionType.Verify);
    }

    void setToken(TRXHOSTSPaymentInfo paymentInfo) {
        paymentInfo.setToken("token");
    }

    void setReceiptData(TRXHOSTSPaymentInfo paymentInfo) {
        final String RECEIPT_DATA = "receipt data";
        paymentInfo.setReceiptData(RECEIPT_DATA);
    }

    void setRecurrent(TRXHOSTSPaymentInfo paymentInfo) {
        TRXHOSTSRecurrentInfo recurrentInfo = new TRXHOSTSRecurrentInfo(
                "R", // type
                "20", // expiry day
                "11", // expiry month
                "2030", // expiry year
...
//        recurrentInfo.setSchedule(new TRXHOSTSRecurrentInfoSchedule[]{
//                new TRXHOSTSRecurrentInfoSchedule("20-10-2020",1000),
//                new TRXHOSTSRecurrentInfoSchedule("20-10-2020",1000)
//        });
    ...
    void setKnownAdditionalFields(TRXHOSTSPaymentInfo paymentInfo) {
        paymentInfo.setTRXHOSTSAdditionalFields(new TRXHOSTSAdditionalField[]{
                new TRXHOSTSAdditionalField(TRXHOSTSAdditionalFieldEnums.AdditionalFieldType.customer_first_name, "Mark"),
                new TRXHOSTSAdditionalField(TRXHOSTSAdditionalFieldEnums.AdditionalFieldType.billing_country, "US"),
        });
    }

    // if you want to hide the saved cards, pass the value - true
    void setHideSavedWallets(TRXHOSTSPaymentInfo paymentInfo) {
        paymentInfo.setHideSavedWallets(false);
    }

    // For forced opening of the payment method, pass its code. Example: qiwi, card ...
    void setForcePaymentMethod(TRXHOSTSPaymentInfo paymentInfo) {
        paymentInfo.setForcePaymentMethod("card");
    }

    // Setup 3D Secure 2.0 parameters
    void setThreeDSecureParams(TRXHOSTSPaymentInfo paymentInfo) {
        TRXHOSTSThreeDSecureInfo threeDSecureInfo = new TRXHOSTSThreeDSecureInfo();

        TRXHOSTSThreeDSecurePaymentInfo threeDSecurePaymentInfo = new TRXHOSTSThreeDSecurePaymentInfo();
        ...
        TRXHOSTSThreeDSecureGiftCardInfo threeDSecureGiftCardInfo = new TRXHOSTSThreeDSecureGiftCardInfo();
        ...
        TRXHOSTSThreeDSecureCustomerInfo threeDSecureCustomerInfo = new TRXHOSTSThreeDSecureCustomerInfo();
        ...
        TRXHOSTSThreeDSecureAccountInfo threeDSecureAccountInfo = new TRXHOSTSThreeDSecureAccountInfo();
        ...
        TRXHOSTSThreeDSecureShippingInfo threeDSecureShippingInfo = new TRXHOSTSThreeDSecureShippingInfo();
        ...
        TRXHOSTSThreeDSecureMpiResultInfo threeDSecureMpiResultInfo = new TRXHOSTSThreeDSecureMpiResultInfo();
        ...
        paymentInfo.setTRXHOSTSThreeDSecureInfo(threeDSecureInfo);
    }
}
